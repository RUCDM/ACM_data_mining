...
//	
	...
	public void initialize(
		...
		this.alpha_=alpha;
		this.userNum_=getUserNum(fileTestData);
...
//		this.eta_=eta;
	...
	public void recommendQuestion() throws IOException{
		...
		for(int u=startUser;u<endUser;u++){
			...
			eta=calculateEtaLast(this.dataTrain_[u],this.indexToTerm_,5);
...
//		System.out.println("MAP@10"+":"+MAPTop10/userNum);
	...
	private double calculateEtaLast(int[] data,int[] indexToTerm,int last){
		int length=data.length;
		double prior=0.0;
		int[] problemList=new int[length];
		for(int i=0;i<length;i++){
		...
		return (1.0*numV+prior)/(lengthAll+2*prior);
	}
